...
class LineFeatureTracker
{
  public:
    ...
    void readImage4Line(const Mat &_img, double _cur_time, const Matrix3d &_relative_R = Matrix3d::Identity());
    ...
    void lineMergingTwoPhaseUseIMU( Mat &prev_img, Mat &cur_img, vector<LineKL> &prev_keyLine, vector<LineKL> &cur_keyLine, Mat &prev_descriptor, Mat &cur_descriptor, vector<DMatch> &good_match_vector, const Matrix3d &_relative_R );
    ...
    static void reduceVector4Line(vector<Point2f> &v, vector<uchar> status);
    ...
    //vector<line_descriptor::KeyLine> predict_keylines, unextracted_lines;
