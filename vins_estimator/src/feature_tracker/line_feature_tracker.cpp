...
void LineFeatureTracker::reduceVector4Line(vector<Point2f> &v, vector<uchar> status)
{
    int j = 0;
...
bool LineFeatureTracker::inBorder(const cv::Point2f &pt)
{
...
void LineFeatureTracker::readImage4Line(const Mat &_img, double _cur_time, const Matrix3d &_relative_R)
{
    TicToc t_r;
    Mat img;
    cur_time = _cur_time;
...
//        double t_linematching = t_r.toc() - t_lineextraction;
        if(USE_IMU){
            lineMergingTwoPhaseUseIMU( curr_img, forw_img, curr_keyLine, forw_keyLine,
                                 curr_descriptor, forw_descriptor, good_match_vector, _relative_R );
        }
        else{
            lineMergingTwoPhase( curr_img, forw_img, curr_keyLine, forw_keyLine,
                                 curr_descriptor, forw_descriptor, good_match_vector );
        }
                                             ...
                                             Mat &prev_descriptor, Mat &cur_descriptor, vector<DMatch> &good_match_vector )
{
    ...
    if(line_distribution == 2)
    {
        ...
        reduceVector4Line(cur_pts, status);
        reduceVector4Line(forw_pts, status);
    }
    else
    {
...
void LineFeatureTracker::lineMergingTwoPhaseUseIMU( Mat &prev_img, Mat &cur_img, vector<LineKL> &prev_keyLine, vector<LineKL> &cur_keyLine,
                                             Mat &prev_descriptor, Mat &cur_descriptor, vector<DMatch> &good_match_vector, const Matrix3d &_relative_R )
{
    TicToc t_linemerging;
    int line_split = true;
    vector<uchar> temp_status;
    ...
    for (unsigned int i = 0; i < cur_pts.size(); ++i)
    {
        ...
        forw_pts[i].x = tmp_p.x();
        forw_pts[i].y = tmp_p.y();
    }
                                               ...
                                               Mat &prev_descriptor, Mat &cur_descriptor)
{
    ...
    for( int i = 0; i < forw_pts.size(); i+=line_distribution ){
        ...
        for(int j = 0; j < line_distribution; j++)
        {
            if(!inBorder(forw_pts[i + j]) || !status[i + j])
            {
                isBad = true;
                break;
            }
    ...
    reduceVector(ids, status_reduced);
    reduceVector(track_cnt, status_reduced);
